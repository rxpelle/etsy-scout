"""Configuration management for Etsy Scout.
...
class Config:
    """Central configuration for Etsy Scout."""

    # Database
    DB_PATH = os.getenv('DB_PATH', 'data/etsy_scout.db')

    # Etsy API (optional, for enhanced data)
    ...
    LISTING_SCRAPE_RATE_LIMIT = float(os.getenv('LISTING_SCRAPE_RATE_LIMIT', '2.0'))
    ...
    def as_dict(cls):
        ...
        return {
            ...
            'ETSY_API_KEY': '***' if cls.ETSY_API_KEY else '(not set)',
            'PROXY_URL': cls.PROXY_URL or '(not set)',
            'AUTOCOMPLETE_RATE_LIMIT': f'{cls.AUTOCOMPLETE_RATE_LIMIT}s',
            'LISTING_SCRAPE_RATE_LIMIT': f'{cls.LISTING_SCRAPE_RATE_LIMIT}s',
